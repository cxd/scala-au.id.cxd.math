...
This implementation is provided by: http://tartarus.org/martin/PorterStemmer/scala.txt
...
class PorterStemmer  {
  ...
  */
  ...
  /**
    * execute the stemming process check if the text length is greater than 2
    * @return
    */
  def stem() = {
    if (b.length > 2) {
