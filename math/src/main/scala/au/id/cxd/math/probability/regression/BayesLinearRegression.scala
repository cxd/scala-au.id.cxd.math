...
class BayesLinearRegression(@transient var inX: DenseMatrix[Double], @transient var inY: DenseVector[Double], override val m: Int = 1)
  ...
  def update(newX:DenseMatrix[Double], newY:DenseVector[Double]) = {
    ...
    val newY1 = DenseMatrix.tabulate(newY.length, 1) { case (i, j) => newY(i) }
    ...
    /**
      ...
      **/
    val newI = DenseMatrix.tabulate(pseudoInverse.rows, pseudoInverse.cols) {
      case (i,j) => i == j match {
   ...
   result2$B <- as.matrix(Mu2)
    ...
    val meanVec = DenseMatrix.ones[Double](mu0.rows, 1) * mu
    val invLambda1 = PseudoInverse(lambda0)
    val invLambda2 = PseudoInverse(lambdaNew)
    val invCov = PseudoInverse(cov)
    val mat1 = invLambda1 + newP.rows * invLambda2
    val mat2 = invLambda1*mu0 + (newP.rows * invCov)* meanVec
    val newBeta = PseudoInverse(mat1) * PseudoInverse(mat2)
