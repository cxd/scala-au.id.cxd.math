...
When you have several dependent variables and several samples/groups the four statistics that may be used to identify differences between group means are Pillai’s trace, Wilk’s Lambda, Roy’s largest root, and Lawes-Hotelling trace. Briefly, describe and compare.

The four statistics are derived from the
...
For the four statistics used in the MANOVA testing, all tests require multivariate normality, so prior to testing check the data for this assumption.
